...
const AGENT_ID = process.env.AGENTMEMORY_AGENT_ID || "";
...
async function callAgentMemory<T>(
  ...
  let url = `${baseUrl}/agentmemory/${pathname.replace(/^\/+/, "")}`;
  if (AGENT_ID) url += `${url.includes("?") ? "&" : "?"}agentId=${encodeURIComponent(AGENT_ID)}`;
    ...
    const response = await fetch(url, {
      ...
      body:
        options?.body !== undefined
          ? JSON.stringify(
              AGENT_ID && typeof options.body === "object" && options.body !== null && !Array.isArray(options.body)
                ? { agentId: AGENT_ID, ...(options.body as Record<string, unknown>) }
                : options.body,
            )
          : undefined,
    });
